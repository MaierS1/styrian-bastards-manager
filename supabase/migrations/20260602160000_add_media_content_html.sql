...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'media_items_content_html_not_blank'
      and conrelid = 'public.media_items'::regclass
  ) then
    alter table public.media_items
      add constraint media_items_content_html_not_blank
      check (content_html is null or length(trim(content_html)) > 0);
  end if;
end;
$$;
