create or replace function public.create_public_shop_order(
  ...
  p_merch_variant_id uuid default null
)
returns table (
  shop_order_id uuid,
...
begin
  ...
  if p_merch_variant_id is not null then
    select *
      into v_variant
    from public.merch_variants
    where id = p_merch_variant_id
      and merch_item_id = v_item.id
      and status = 'active'
      and is_public = true
    ...
    if not found then
      raise exception 'public merch variant not found or requested quantity is not available';
    end if;
  else
    select *
      into v_variant
...
revoke all on function public.create_public_shop_order(uuid, text, text, text, integer, text, text, text, uuid) from public;
grant execute on function public.create_public_shop_order(uuid, text, text, text, integer, text, text, text, uuid) to anon;
grant execute on function public.create_public_shop_order(uuid, text, text, text, integer, text, text, text, uuid) to authenticated;
